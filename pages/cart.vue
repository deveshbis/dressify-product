<template>
  <Navbar />
  <div class="font-sans max-w-6xl max-md:max-w-xl mx-auto p-4">
        ...
        <div v-for="item in cartItems" :key="item.id"
          ...
          <div class="flex gap-4">
            ...
            <div class="flex flex-col gap-4">
              ...
              <div class="mt-auto flex items-center gap-3">
                <button @click="decrementQuantity(item)" type="button"
                ...
                <button @click="incrementQuantity(item)" type="button"
          ...
          <div class="ml-auto flex flex-col">
            <div class="flex items-start gap-4 justify-end">
              <svg @click="addToWishlist(item)" xmlns="http://www.w3.org/2000/svg"
              ...
              <svg @click="handleDelete(item.id)" xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="bg-white rounded-md px-4 py-6 h-max shadow-[0_2px_12px_-3px_rgba(61,63,68,0.3)]">
        <ul class="text-gray-800 space-y-4">
          ...
          <li class="flex flex-wrap gap-4 text-sm font-bold">
            Total <span class="ml-auto">${{ calculateTotal() }}</span>
          </li>
        </ul>
...
<script>
import { handleDelete, addToWishlist } from "~/utils/cart";
...
export default {
  ...
  methods: {
    ...
    handleDelete(itemId) {
      handleDelete(itemId, this.cartItems);
    },
    ...
    calculateSubtotal() {
      return this.cartItems
        .reduce((total, item) => total + (parseFloat(item.price) * parseInt(item.quantity)), 0)
        .toFixed(2);
    },
